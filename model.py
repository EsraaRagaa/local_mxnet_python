...
class FeedForward(BASE_ESTIMATOR):
    ...
    def fit(self, X, y=None, eval_data=None, eval_metric='acc',
        ...
        data_info = data.provide_data + data.provide_label
        for info_index, info in enumerate(data_info):
          index = info.layout.index('N')
          N = info.shape[index]
          shape = info.shape[:index] + (N / len(self.ctx),) + info.shape[index + 1:]
          data_info[info_index] = io.DataDesc(info.name, shape, info.dtype, info.layout)
          
        arg_names, param_names, aux_names = \
                self._init_params(dict(data_info))
